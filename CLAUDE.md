...
## Data models
- **User** — auth + profile. Also holds `emailReminders` (opt-in daily habit reminder prefs: `enabled`, `mode: 'all' | 'specific'`, `habitIds`)
...
- Notifications: `PATCH /api/auth/notifications` — updates `emailReminders` (`enabled`, `mode`, `habitIds`); `GET /api/auth/me` echoes it back (signup/login responses don't include it)
- Cron (machine-to-machine, no JWT): `POST /api/cron/habit-reminders` — guarded by an `x-cron-secret` header matched against `CRON_SECRET` (see `server/middleware/cronAuth.js`), not a user session. The only endpoint in the app with no client behind it — see the day-boundary note below for why that matters.
...
12. ✅ Email reminders (2026-07-21) — opt-in daily email if any habits are still undone, via Resend, on a fixed schedule (11pm IST / 17:30 UTC for now — no per-user time picker yet, see below):
    - Backend: `emailReminders` prefs on `User` (`enabled` / `mode: 'all' | 'specific'` / `habitIds`), `PATCH /api/auth/notifications`, `server/services/reminders.service.js` (selects who's due + which of their habits are still undone today), `server/services/email.service.js` (Resend wrapper), `POST /api/cron/habit-reminders` guarded by `server/middleware/cronAuth.js` (shared-secret, not JWT — see API conventions above)
    - Since Render's free tier spins the backend down when idle, nothing inside it can run its own cron — `.github/workflows/habit-reminders.yml` is the external trigger: a GitHub Actions scheduled workflow that calls the cron endpoint once a day (plus a manual `workflow_dispatch` button for testing)
    - Frontend: a new "Email reminders" card in `SettingsPage.tsx` — on/off toggle, `all`/`specific` mode, and a habit checklist when `specific` is chosen
    - Deliberately scoped to habits only (not tasks/reflections) and one fixed send time for everyone — a per-user time picker would need the job to run hourly and filter by each user's chosen hour, not worth the complexity for a single-user app right now

## Testing
- **Backend:** `cd server && npm test` — Jest + Supertest + mongodb-memory-server; tests in `server/__tests__/` (9 suites, 58 tests)
